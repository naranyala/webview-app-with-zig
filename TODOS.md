...
- [x] Connect `Quiz Editor` to the versioned backend quiz store for custom decks and question CRUD; keep bundled example collections read-only. Implemented in `src/backend/quiz_storage.zig`, the seven quiz bindings, and `frontend-preact/src/plugins/quiz.jsx`.
- [ ] Expand Quiz collections and add a content authoring/schema workflow so new decks do not require editing the component.
- [ ] Persist Quiz progress through the versioned backend store; custom collections and questions now persist in `quizzes.json`, while bundled decks remain read-only and ids are generated by the backend.
...
- [x] Single-source the binding list: `frontend-preact/check-bindings.cjs` derives its expectations from `src/backend/core_plugin.zig:bound_names` instead of maintaining a duplicate list by hand.

## Integration

- [x] Keep `bindings.d.ts` synchronized with Zig bindings, ideally through generated types or a shared API schema. Implemented: `src/backend/core_plugin.zig:bound_names` (21 names + uniqueness test) + `frontend-preact/check-bindings.cjs` (`npm run check:bindings`, wired into `npm run build`, `zig build test`, and CI).
...
- [x] Make `run.sh` call `tools/check-deps.sh` (and require node/npm) before building so missing toolchains fail with a clear message instead of an obscure `npm` error.
